<script lang="ts">
	import { evidenceItems, evidenceStatusInfo, auditSummary } from '$lib/data/pb-evidence-audit-data';
	import { chain, getWeakestPath, chainSummary, typeLabels as chainTypeLabels } from '$lib/data/pb-argument-chain-data';
	import PbArgumentGraph from '$lib/components/PbArgumentGraph.svelte';
...
<svelte:head>
	<title>Evidenz-Audit &amp; Argumentationskette — Pink Brain, Blue Brain</title>
...
<div class="app">
	<header class="hero">
		<div class="hero-inner">
			<div class="nav-links">
				<a href="/pink-brain-blue-brain" class="back-link">&larr; Buch&uuml;bersicht</a>
				<a href="/pink-brain-blue-brain/references" class="back-link">Quellenverzeichnis &rarr;</a>
			</div>
			<p class="book-label">Evidenz-Audit &amp; Argumentationskette</p>
			<h1>Wie belastbar ist das Fundament?</h1>
			<p class="subtitle">
				Evidenz-Audit und Argumentationskette f&uuml;r
				<em>Pink Brain, Blue Brain</em>
			</p>
	...
	<section class="chain-section">
		...
		<p class="chain-intro">
			Die Schritte der Buchlogik — von den Pr&auml;missen bis zu den Handlungsempfehlungen.
			Jeder Schritt zeigt an, auf welchem Vorg&auml;nger er aufbaut. Rote und gelbe Glieder schw&auml;chen alles, was auf ihnen steht.
		</p>
		...
		<!-- Interactive graph -->
		<PbArgumentGraph onNodeClick={(id) => (selectedNodeId = selectedNodeId === id ? null : id)} />
		...
		{#if selectedNode}
			...
			<div class="node-detail" style="--nd-color: {color}">
				...
				{#if selectedNode.dependsOn.length > 0}
					<div class="nd-deps">
						<span class="nd-deps-label">St&uuml;tzt sich auf:</span>
				...
				<div class="nd-evidence">
					...
					{#each selectedNode.evidenceRefs as ref, i}
						<a class="ev-link" href="/pink-brain-blue-brain/references?search={encodeURIComponent(ref.authorSearch)}">{ref.label}</a>{#if i < selectedNode.evidenceRefs.length - 1}<span class="ev-sep">&middot;</span>{/if}
					{/each}
				</div>
	...
	<section class="audit-section">
		...
		<p class="audit-intro">
			...
			Replikationsstand und die Methodenkritik gepr&uuml;ft.
		</p>
				...
				<div class="audit-item" style="--item-color: {info.color}">
					<div class="audit-item-header">
						...
						<span class="centrality-tag centrality-{item.centrality}">
							{item.centrality === 'critical' ? 'Tragend' : item.centrality === 'supporting' ? 'St&uuml;tzend' : 'Nebens&auml;chlich'}
						</span>
					</div>
	...
	<footer class="app-footer">
		<p>
			Evidenz-Audit und Argumentationsanalyse zu <em>Pink Brain, Blue Brain</em> (Eliot, 2009).
...
<style>
	...
	.show-all-btn:hover { background: rgba(236, 72, 153, 0.2); }
