<script lang="ts">
	import { evidenceItems, evidenceStatusInfo, auditSummary } from '$lib/data/nbm-evidence-audit-data';
	import { chain, getWeakestPath, chainSummary, typeLabels as chainTypeLabels } from '$lib/data/nbm-argument-chain-data';
	import NbmArgumentGraph from '$lib/components/NbmArgumentGraph.svelte';
...
<svelte:head>
	<title>Evidenz-Audit &amp; Argumentationskette — Notes on Being a Man</title>
...
<div class="app">
	<header class="hero">
		<div class="hero-inner">
			<div class="nav-links">
				<a href="/notes-on-being-a-man" class="back-link">&larr; Buchübersicht</a>
				<a href="/notes-on-being-a-man/references" class="back-link">Quellenverzeichnis &rarr;</a>
			</div>
			<p class="book-label">Evidenz-Audit &amp; Argumentationskette</p>
			<h1>Vom Memoir zur Männlichkeits-Ethik</h1>
			<p class="subtitle">
				Evidenz-Audit und Argumentationskette für
				<em>Notes on Being a Man</em>
			</p>
			<p class="subtitle-note">
				Ausdrücklich &bdquo;observations, not peer-reviewed academic research&ldquo;. Wo Galloway
				Reeves und Regierungsstatistiken nacherzählt, ist er solide; wo er selbst recherchiert,
				zitiert er Medium-Posts, Glamour-Artikel und pseudonyme Blogs &mdash; bis hin zu einer
				zurückgezogenen Studie als Chart-Quelle.
			</p>
		</div>
	</header>

	<section class="chain-section">
		<h2>Argumentationskette: Von der Krisen-Diagnose zur Aspirational Masculinity</h2>
		<p class="chain-intro">
			Galloways Schritte &mdash; von der Krisen-Diagnose (Deaths of Despair, NEET, Mating-Rezession)
			und der kulturellen Preisgabe der Männer über die biologische Prämisse (Testosteron,
			PFC-Timing &mdash; inklusive des roten Brizendine/Diaz-&amp;-Bailey-Glieds) und die Mechanismen
			(Addiction Economy, Porn, Guardrails, Friendship Recession) zum normativen Kern:
			Protect/Provide/Procreate, Surplus Value, die Scott-Methode und der Galloway-Code.
		</p>
		...
		<NbmArgumentGraph onNodeClick={(id) => (selectedNodeId = selectedNodeId === id ? null : id)} />
			...
			<div class="node-detail" style="--nd-color: {color}">
				...
				<div class="nd-evidence">
					...
					{#each selectedNode.evidenceRefs as ref, i}
						<a class="ev-link" href="/notes-on-being-a-man/references?search={encodeURIComponent(ref.authorSearch)}">{ref.label}</a>{#if i < selectedNode.evidenceRefs.length - 1}<span class="ev-sep">&middot;</span>{/if}
					{/each}
				</div>
	...
	<section class="audit-section">
		<h2>Evidenz-Audit: Was trägt der Podcast-Professor-Fußnotenapparat?</h2>
		<p class="audit-intro">
			Galloway mischt drei Quellenklassen: (1) das Reeves/AIBM-Ökosystem und Regierungsstatistiken
			(solide), (2) große Studien aus zweiter Hand (Chetty, Holt-Lunstad, Noetel &mdash; korrekt
			referiert) und (3) journalistische Sekundärquellen (Medium, Glamour, Quartz, Anwalts-Blogs)
			für die zugespitzten Zahlen. Prüfregel: Jede Statistik gegen die Primärquelle checken,
			bevor man sie weiterzitiert.
		</p>
	...
	<footer class="app-footer">
		<p>
			Evidenz-Audit und Argumentationsanalyse zu <em>Notes on Being a Man</em>
			(Scott Galloway, Simon &amp; Schuster 2025). Das Selbsthilfe-Memoir des Korpus: inhaltlich
			weitgehend ein Reeves-Derivat mit biologistischerem Einschlag, das die klinische Konvergenz
			der Bibliothek (Maske, Alexithymie, Emotionsverlust) in der ersten Person bestätigt &mdash;
			während es deren Vokabular (&bdquo;toxische Männlichkeit&ldquo;) lautstark verwirft.
		</p>
	</footer>
</div>
...
<style>
	...
	.show-all-btn:hover { background: rgba(16, 185, 129, 0.2); }
