<script lang="ts">
	import { evidenceItems, evidenceStatusInfo, auditSummary } from '$lib/data/gm-evidence-audit-data';
	import { chain, getWeakestPath, chainSummary, typeLabels as chainTypeLabels } from '$lib/data/gm-argument-chain-data';
	import GmArgumentGraph from '$lib/components/GmArgumentGraph.svelte';
...
<svelte:head>
	<title>Evidenz-Audit &amp; Argumentationskette — Der gemachte Mann</title>
...
<div class="app">
	<header class="hero">
		<div class="hero-inner">
			<div class="nav-links">
				...
				<a href="/der-gemachte-mann/references" class="back-link">Quellenverzeichnis &rarr;</a>
			</div>
			<p class="book-label">Evidenz-Audit &amp; Argumentationskette</p>
			<h1>Wie belastbar ist Connells Fundament?</h1>
			<p class="subtitle">
				Evidenz-Audit und Argumentationskette für
				<em>Der gemachte Mann</em>
			</p>
			<p class="subtitle-note">
				Wo ist Connells Argumentation empirisch solide, wo theoretischer Rahmen,
				wo politisch interpretiert? Welche Glieder tragen den Rest der Konstruktion?
			</p>
		</div>
	</header>
	...
	<section class="chain-section">
		...
		<p class="chain-intro">
			Die Schritte von Connells theoretischer Architektur &mdash; von den fundamentalen
			Ablehnungen (Biologie, Rollentheorie) über das Mehrebenenmodell der Geschlechterstruktur
			und die Typologie der Männlichkeiten bis zur strategischen Synthese. Jeder Schritt zeigt,
			auf welchen Vorgängern er aufbaut.
		</p>
		...
		<!-- Interactive graph -->
		<GmArgumentGraph onNodeClick={(id) => (selectedNodeId = selectedNodeId === id ? null : id)} />
			...
			<div class="node-detail" style="--nd-color: {color}">
				...
				<div class="nd-evidence">
					...
					{#each selectedNode.evidenceRefs as ref, i}
						<a class="ev-link" href="/der-gemachte-mann/references?search={encodeURIComponent(ref.authorSearch)}">{ref.label}</a>{#if i < selectedNode.evidenceRefs.length - 1}<span class="ev-sep">&middot;</span>{/if}
					{/each}
				</div>
	...
	<section class="audit-section">
		...
		<p class="audit-intro">
			Jede tragende Behauptung wurde gegen den aktuellen Forschungsstand und die theoretischen
			Kritiken (Donaldson 1993, Hearn 2004, Beasley 2008, Connell &amp; Messerschmidt 2005) geprüft.
		</p>
	...
	<footer class="app-footer">
		<p>
			Evidenz-Audit und Argumentationsanalyse zu <em>Der gemachte Mann</em>
			(Raewyn Connell, Springer VS 4. Aufl. 2015 / engl. Orig. <em>Masculinities</em> 1995/2005).
			Die Einordnungen basieren auf dem Stand der Männlichkeitsforschung und der Diskussion
			um hegemoniale Männlichkeit (Connell &amp; Messerschmidt 2005, Hearn 2004, Beasley 2008,
			Donaldson 1993) bis 2025.
		</p>
	</footer>
</div>
...
<style>
	...
	.show-all-btn:hover { background: rgba(167, 139, 250, 0.2); }
