<script lang="ts">
	import { evidenceItems, evidenceStatusInfo, auditSummary } from '$lib/data/pr-evidence-audit-data';
	import { chain, getWeakestPath, chainSummary, typeLabels as chainTypeLabels } from '$lib/data/pr-argument-chain-data';
	import PrArgumentGraph from '$lib/components/PrArgumentGraph.svelte';
...
<svelte:head>
	<title>Evidenz-Audit &amp; Argumentationskette — Was Sie (Wirklich) Über Erziehung Wissen Müssen</title>
...
<div class="app">
	<header class="hero">
		<div class="hero-inner">
			<div class="nav-links">
				<a href="/was-sie-wirklich" class="back-link">&larr; Buchübersicht</a>
				<a href="/was-sie-wirklich/references" class="back-link">Quellenverzeichnis &rarr;</a>
			</div>
			...
			<p class="subtitle">
				...
				<em>Was Sie (Wirklich) Über Erziehung Wissen Müssen</em>
			</p>
	...
	<section class="chain-section">
		...
		<!-- Interactive graph -->
		<PrArgumentGraph onNodeClick={(id) => (selectedNodeId = selectedNodeId === id ? null : id)} />
			...
			<div class="node-detail" style="--nd-color: {color}">
				...
				<div class="nd-evidence">
					...
					{#each selectedNode.evidenceRefs as ref, i}
						<a class="ev-link" href="/was-sie-wirklich/references?search={encodeURIComponent(ref.authorSearch)}">{ref.label}</a>{#if i < selectedNode.evidenceRefs.length - 1}<span class="ev-sep">&middot;</span>{/if}
					{/each}
				</div>
	...
	<footer class="app-footer">
		<p>
			Evidenz-Audit und Argumentationsanalyse zu <em>Was Sie (Wirklich) Über Erziehung Wissen Müssen</em> (Prüfer, 2026).
...
<style>
	...
	.show-all-btn:hover { background: rgba(245, 158, 11, 0.2); }
