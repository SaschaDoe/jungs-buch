<script lang="ts">
	import { evidenceItems, evidenceStatusInfo, auditSummary } from '$lib/data/mse-evidence-audit-data';
	import { chain, getWeakestPath, chainSummary, typeLabels as chainTypeLabels } from '$lib/data/mse-argument-chain-data';
	import MseArgumentGraph from '$lib/components/MseArgumentGraph.svelte';
...
<svelte:head>
	<title>Evidenz-Audit &amp; Argumentationskette — Männerseelen</title>
...
<div class="app">
	<header class="hero">
		<div class="hero-inner">
			<div class="nav-links">
				<a href="/maennerseelen" class="back-link">&larr; Buchübersicht</a>
				<a href="/maennerseelen/references" class="back-link">Quellenverzeichnis &rarr;</a>
			</div>
			<p class="book-label">Evidenz-Audit &amp; Argumentationskette</p>
			<h1>Von der Alexithymie zur Sehnsucht nach sich selbst</h1>
			<p class="subtitle">
				Evidenz-Audit und Argumentationskette für
				<em>Männerseelen</em>
			</p>
			<p class="subtitle-note">
				Ein klinisch-phänomenologisches Buch. Die meisten zentralen Begriffe sind klinisch-beratungspraktisch
				konvergent validiert — das ist Programm, nicht Schwäche.
			</p>
		</div>
	</header>

	<section class="chain-section">
		<h2>Argumentationskette: Vom Rogers-Fundament zur liebevollen Konfrontation</h2>
		<p class="chain-intro">
			Süfkes Schritte &mdash; vom humanistisch-neuropsychotherapeutischen Fundament (Rogers, Grawe)
			über den 4-Etappen-Weg und die Sozialisationsmechanismen (mangelnde Spiegelung, Umweg-Identifikation)
			zu den klinischen Phänomenen (Alexithymie, Identitätslüge, Hilflosigkeit, verdeckte Depression, Impotenz)
			und schließlich zur therapeutischen Methode (liebevolle Konfrontation, Sehnsucht nach sich selbst).
		</p>
		...
		<MseArgumentGraph onNodeClick={(id) => (selectedNodeId = selectedNodeId === id ? null : id)} />
			...
			<div class="node-detail" style="--nd-color: {color}">
				...
				<div class="nd-evidence">
					...
					{#each selectedNode.evidenceRefs as ref, i}
						<a class="ev-link" href="/maennerseelen/references?search={encodeURIComponent(ref.authorSearch)}">{ref.label}</a>{#if i < selectedNode.evidenceRefs.length - 1}<span class="ev-sep">&middot;</span>{/if}
					{/each}
				</div>
	...
	<section class="audit-section">
		<h2>Evidenz-Audit: Was trägt die klinische Männertherapie?</h2>
		<p class="audit-intro">
			Süfke arbeitet mit einer Kombination aus klinischer Praxis (Männerberatung Bielefeld),
			theoretischem Fundament (Rogers, Grawe), psychoanalytischer Geschlechtstheorie (Chodorow),
			deutscher Männerforschung (Böhnisch) und expliziter Aufnahme der amerikanischen klinischen
			Männerliteratur (Real, Zilbergeld).
		</p>
	...
	<footer class="app-footer">
		<p>
			Evidenz-Audit und Argumentationsanalyse zu <em>Männerseelen — Ein psychologischer Reiseführer</em>
			(Björn Süfke, Gütersloher Verlagshaus 2010). Süfke ist der klinisch-phänomenologische Spiegel
			zur strukturtheoretischen Grundlage Böhnischs — kompatibel und ergänzend.
		</p>
	</footer>
</div>
...
<style>
	...
	.show-all-btn {
		...
		border-radius: 8px; color: #22d3ee; font-size: 0.8rem;
		cursor: pointer; font-family: inherit;
	}
	.show-all-btn:hover { background: rgba(34, 211, 238, 0.2); }
