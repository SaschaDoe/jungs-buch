<script lang="ts">
	import { evidenceItems, evidenceStatusInfo, auditSummary } from '$lib/data/wb-evidence-audit-data';
	import { chain, getWeakestPath, chainSummary, typeLabels as chainTypeLabels } from '$lib/data/wb-argument-chain-data';
	import WbArgumentGraph from '$lib/components/WbArgumentGraph.svelte';
...
<svelte:head>
	<title>Evidenz-Audit &amp; Argumentationskette — The Wonder of Boys</title>
...
<div class="app">
	<header class="hero">
		<div class="hero-inner">
			<div class="nav-links">
				<a href="/wonder-of-boys" class="back-link">&larr; Buchübersicht</a>
				<a href="/wonder-of-boys/references" class="back-link">Quellenverzeichnis &rarr;</a>
			</div>
	...
	<section class="chain-section">
		...
		<!-- Interactive graph -->
		<WbArgumentGraph onNodeClick={(id) => (selectedNodeId = selectedNodeId === id ? null : id)} />
			...
			<div class="node-detail" style="--nd-color: {color}">
				...
				<div class="nd-evidence">
					...
					{#each selectedNode.evidenceRefs as ref, i}
						<a class="ev-link" href="/wonder-of-boys/references?search={encodeURIComponent(ref.authorSearch)}">{ref.label}</a>{#if i < selectedNode.evidenceRefs.length - 1}<span class="ev-sep">&middot;</span>{/if}
					{/each}
				</div>
	...
	<footer class="app-footer">
		<p>
			Evidenz-Audit und Argumentationsanalyse zu <em>The Wonder of Boys</em> (Gurian, 1996).
...
<style>
	...
	.show-all-btn:hover { background: rgba(6, 182, 212, 0.2); }
