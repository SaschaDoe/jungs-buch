<script lang="ts">
	import { evidenceItems, evidenceStatusInfo, auditSummary } from '$lib/data/ms-evidence-audit-data';
	import { chain, getWeakestPath, chainSummary, typeLabels as chainTypeLabels } from '$lib/data/ms-argument-chain-data';
	import MsArgumentGraph from '$lib/components/MsArgumentGraph.svelte';
...
<svelte:head>
	<title>Evidenz-Audit &amp; Argumentationskette — Männliche Sozialisation</title>
...
<div class="app">
	<header class="hero">
		<div class="hero-inner">
			<div class="nav-links">
				<a href="/maennliche-sozialisation" class="back-link">&larr; Buchübersicht</a>
				<a href="/maennliche-sozialisation/references" class="back-link">Quellenverzeichnis &rarr;</a>
			</div>
			<p class="book-label">Evidenz-Audit &amp; Argumentationskette</p>
			<h1>Bewältigung und Verdeckung</h1>
			<p class="subtitle">
				Evidenz-Audit und Argumentationskette für
				<em>Männliche Sozialisation</em>
			</p>
			<p class="subtitle-note">
				Ein theoretisch-synthetisches und sozialpädagogisch-diagnostisches Werk. Die meisten
				zentralen Begriffe sind theoretische Rahmen oder klinisch-beratungspraktisch validiert &mdash;
				das ist Programm, nicht Schwäche.
			</p>
		</div>
	</header>

	<section class="chain-section">
		<h2>Argumentationskette: Von der Zweiten Moderne zum balancierten Mann-Sein</h2>
		<p class="chain-intro">
			Böhnischs Schritte &mdash; von der zeitdiagnostischen Rahmung (Zweite Moderne, Externalisierung)
			über das theoretische Leitkonzept (Dominanz UND Verfügbarkeit) und die zentralen Mechanismen
			(Bewältigungsparadigma, Bedürftigkeit) zu den Lebensphasen und dem normativen Ziel des
			balancierten Mann-Seins.
		</p>
		...
		<MsArgumentGraph onNodeClick={(id) => (selectedNodeId = selectedNodeId === id ? null : id)} />
			...
			<div class="node-detail" style="--nd-color: {color}">
				...
				<div class="nd-evidence">
					...
					{#each selectedNode.evidenceRefs as ref, i}
						<a class="ev-link" href="/maennliche-sozialisation/references?search={encodeURIComponent(ref.authorSearch)}">{ref.label}</a>{#if i < selectedNode.evidenceRefs.length - 1}<span class="ev-sep">&middot;</span>{/if}
					{/each}
				</div>
	...
	<section class="audit-section">
		<h2>Evidenz-Audit: Was trägt die deutsche Männersoziologie?</h2>
		<p class="audit-intro">
			Böhnisch arbeitet mit einer Kombination aus eigenen quantitativen Studien (Südtirol, Sachsen),
			qualitativen Interviews, Beratungspraxis (Neumann/Süfke), theoretischer Synthese (Connell,
			Bourdieu, Gruen, Chodorow) und Rezeption der deutschen und internationalen Männerforschung.
		</p>
	...
	<footer class="app-footer">
		<p>
			Evidenz-Audit und Argumentationsanalyse zu <em>Männliche Sozialisation</em>
			(Lothar Böhnisch, Beltz Juventa 2., überarbeitete Auflage 2013). Böhnisch ist die
			akademische deutschsprachige Grundlage, auf der die populäre deutsche Männer-/Jungen-Literatur
			(Dittmann) aufbaut.
		</p>
	</footer>
</div>
...
<style>
	...
	.show-all-btn {
		...
		border-radius: 8px; color: #fbbf24; font-size: 0.8rem;
		cursor: pointer; font-family: inherit;
	}
	.show-all-btn:hover { background: rgba(251, 191, 36, 0.2); }
