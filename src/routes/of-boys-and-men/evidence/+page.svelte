<script lang="ts">
	import { evidenceItems, evidenceStatusInfo, auditSummary } from '$lib/data/obm-evidence-audit-data';
	import { chain, getWeakestPath, chainSummary, typeLabels as chainTypeLabels } from '$lib/data/obm-argument-chain-data';
	import ObmArgumentGraph from '$lib/components/ObmArgumentGraph.svelte';
...
<svelte:head>
	<title>Evidenz-Audit &amp; Argumentationskette — Of Boys and Men</title>
...
<div class="app">
	<header class="hero">
		<div class="hero-inner">
			<div class="nav-links">
				<a href="/of-boys-and-men" class="back-link">&larr; Buchübersicht</a>
				<a href="/of-boys-and-men/references" class="back-link">Quellenverzeichnis &rarr;</a>
			</div>
			<p class="book-label">Evidenz-Audit &amp; Argumentationskette</p>
			<h1>Von der Statistik zur Policy</h1>
			<p class="subtitle">
				Evidenz-Audit und Argumentationskette für
				<em>Of Boys and Men</em>
			</p>
			<p class="subtitle-note">
				Die empirisch robusteste Quelle der gesamten Bibliothek. Fast jede Behauptung wird
				mit einer spezifischen peer-reviewed Studie, Meta-Analyse oder Regierungsstatistik belegt.
				Brookings-Standard: explizite Unsicherheits-Diskussion, offene Anerkennung uneindeutiger Evidenz.
			</p>
		</div>
	</header>

	<section class="chain-section">
		<h2>Argumentationskette: Von der Bildungsgap zur Redshirt-Policy</h2>
		<p class="chain-intro">
			Reeves' Schritte &mdash; von der empirischen Diagnose (Bildungsgap, Arbeitsmarktabstieg,
			Dad Deficit, Black Men Intersection, Deaths of Despair) über den zentralen neurobiologischen
			Erklärungsansatz (Brain-Timing-Gap, Nature AND Nurture) zur politischen Kritik beider Seiten
			(Progressive Blindness, Conservative Rollback) und schließlich zur konkreten Policy-Agenda
			(Redshirt the Boys, Men Can HEAL, Direct Dads).
		</p>
		...
		<ObmArgumentGraph onNodeClick={(id) => (selectedNodeId = selectedNodeId === id ? null : id)} />
			...
			<div class="node-detail" style="--nd-color: {color}">
				...
				<div class="nd-evidence">
					...
					{#each selectedNode.evidenceRefs as ref, i}
						<a class="ev-link" href="/of-boys-and-men/references?search={encodeURIComponent(ref.authorSearch)}">{ref.label}</a>{#if i < selectedNode.evidenceRefs.length - 1}<span class="ev-sep">&middot;</span>{/if}
					{/each}
				</div>
	...
	<section class="audit-section">
		<h2>Evidenz-Audit: Was trägt die Brookings-Wonk-Methode?</h2>
		<p class="audit-intro">
			Reeves arbeitet mit peer-reviewed Meta-Analysen, Regierungsstatistiken (BLS, NCES, CDC),
			großen Längsschnittstudien (Chetty Opportunity Insights, UK Biobank),
			und rigorosen Evaluierungsstudien (Kalamazoo, Cascio-Schanzenbach). Für fast jede Behauptung
			gibt es eine namentlich zitierte Quelle mit methodologischer Beschreibung.
		</p>
	...
	<footer class="app-footer">
		<p>
			Evidenz-Audit und Argumentationsanalyse zu <em>Of Boys and Men</em>
			(Richard V. Reeves, Brookings Institution Press 2022). Die empirisch robusteste Quelle des
			gesamten Bibliothek-Korpus. Als zentristisch-datengetriebene Einzelstimme adjustiert Reeves
			mehrere Konvergenzen der klinischen Tradition (Pollack, Süfke, hooks, Urwin) mit
			Peer-Review-Evidenz.
		</p>
	</footer>
</div>
