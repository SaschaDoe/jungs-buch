<script lang="ts">
	import { evidenceItems, evidenceStatusInfo, auditSummary } from '$lib/data/wgm-evidence-audit-data';
	import { chain, getWeakestPath, chainSummary, typeLabels as chainTypeLabels } from '$lib/data/wgm-argument-chain-data';
	import WgmArgumentGraph from '$lib/components/WgmArgumentGraph.svelte';
...
<svelte:head>
	<title>Evidenz-Audit &amp; Argumentationskette — Why Gender Matters</title>
...
<div class="app">
	<header class="hero">
		<div class="hero-inner">
			<div class="nav-links">
				<a href="/why-gender-matters" class="back-link">&larr; Buchübersicht</a>
				<a href="/why-gender-matters/references" class="back-link">Quellenverzeichnis &rarr;</a>
			</div>
			...
			<p class="subtitle">
				...
				<em>Why Gender Matters, Second Edition</em>
			</p>
	...
	<section class="chain-section">
		...
		<!-- Interactive graph -->
		<WgmArgumentGraph onNodeClick={(id) => (selectedNodeId = selectedNodeId === id ? null : id)} />
			...
			<div class="node-detail" style="--nd-color: {color}">
				...
				<div class="nd-evidence">
					...
					{#each selectedNode.evidenceRefs as ref, i}
						<a class="ev-link" href="/why-gender-matters/references?search={encodeURIComponent(ref.authorSearch)}">{ref.label}</a>{#if i < selectedNode.evidenceRefs.length - 1}<span class="ev-sep">&middot;</span>{/if}
					{/each}
				</div>
	...
	<footer class="app-footer">
		<p>
			Evidenz-Audit und Argumentationsanalyse zu <em>Why Gender Matters, Second Edition</em> (Sax, 2017).
...
<style>
	...
	.show-all-btn:hover { background: rgba(52, 211, 153, 0.2); }
