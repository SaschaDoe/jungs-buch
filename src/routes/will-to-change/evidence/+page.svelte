<script lang="ts">
	import { evidenceItems, evidenceStatusInfo, auditSummary } from '$lib/data/wtc-evidence-audit-data';
	import { chain, getWeakestPath, chainSummary, typeLabels as chainTypeLabels } from '$lib/data/wtc-argument-chain-data';
	import WtcArgumentGraph from '$lib/components/WtcArgumentGraph.svelte';
...
<svelte:head>
	<title>Evidenz-Audit &amp; Argumentationskette — The Will to Change</title>
...
<div class="app">
	<header class="hero">
		<div class="hero-inner">
			<div class="nav-links">
				...
				<a href="/will-to-change/references" class="back-link">Quellenverzeichnis &rarr;</a>
			</div>
			<p class="book-label">Evidenz-Audit &amp; Argumentationskette</p>
			<h1>Eine Liebesethik als Politik</h1>
			<p class="subtitle">
				Evidenz-Audit und Argumentationskette für
				<em>The Will to Change</em>
			</p>
			<p class="subtitle-note">
				Ein bewusst ethisch-theoretisches Buch. Die meisten Knoten sind &bdquo;nicht testbar&ldquo;
				oder &bdquo;wacklig&ldquo; &mdash; und das ist Programm, nicht Schwäche.
			</p>
		</div>
	</header>

	<section class="chain-section">
		<h2>Argumentationskette: Von der Patriarchatskritik zur Liebesethik</h2>
		<p class="chain-intro">
			hooks&rsquo; Schritte &mdash; von der empirischen Grundlage (Säuglinge sind emotional
			ausdrucksstark) über die strukturelle Kritik (Patriarchat als Dominanzsystem, das beide
			Geschlechter mitträgt) bis zur normativen Synthese (Liebesethik, Gemeinschaften des
			Widerstands). Jeder Knoten zeigt, auf welchen Vorgängern er steht.
		</p>
		...
		<WtcArgumentGraph onNodeClick={(id) => (selectedNodeId = selectedNodeId === id ? null : id)} />
			...
			<div class="node-detail" style="--nd-color: {color}">
				...
				<div class="nd-evidence">
					...
					{#each selectedNode.evidenceRefs as ref, i}
						<a class="ev-link" href="/will-to-change/references?search={encodeURIComponent(ref.authorSearch)}">{ref.label}</a>{#if i < selectedNode.evidenceRefs.length - 1}<span class="ev-sep">&middot;</span>{/if}
					{/each}
				</div>
	...
	<section class="audit-section">
		<h2>Evidenz-Audit: Was trägt eine Liebesethik?</h2>
		<p class="audit-intro">
			hooks arbeitet nicht mit Studien, sondern mit theoretischen Stimmen (Terrence Real,
			John Bradshaw, M. Scott Peck, Dorothy Dinnerstein), autobiographischem Material
			und Kulturkritik. Jede Behauptung wurde gegen den Forschungsstand und den theoretischen
			Kontext geprüft.
		</p>
	...
	<footer class="app-footer">
		<p>
			Evidenz-Audit und Argumentationsanalyse zu <em>The Will to Change: Men, Masculinity, and Love</em>
			(bell hooks, Atria Books 2004). hooks ist eine ethische und kulturkritische Stimme, keine
			empirische Forscherin; die Einordnungen reflektieren das bewusst.
		</p>
	</footer>
</div>
...
<style>
	...
	.subtitle-note { color: #64748b; font-size: 0.85rem; margin: 0; max-width: 680px; margin-left: auto; margin-right: auto; line-height: 1.5; }
	...
	.show-all-btn {
		...
		border-radius: 8px; color: #f472b6; font-size: 0.8rem;
		cursor: pointer; font-family: inherit;
	}
	.show-all-btn:hover { background: rgba(244, 114, 182, 0.2); }
