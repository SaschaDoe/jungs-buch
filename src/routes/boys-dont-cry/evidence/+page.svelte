<script lang="ts">
	import { evidenceItems, evidenceStatusInfo, auditSummary } from '$lib/data/bdc-evidence-audit-data';
	import { chain, getWeakestPath, chainSummary, typeLabels as chainTypeLabels } from '$lib/data/bdc-argument-chain-data';
	import BdcArgumentGraph from '$lib/components/BdcArgumentGraph.svelte';
...
<svelte:head>
	<title>Evidenz-Audit &amp; Argumentationskette — Boys Don't Cry</title>
...
<div class="app">
	<header class="hero">
		<div class="hero-inner">
			<div class="nav-links">
				...
				<a href="/boys-dont-cry/references" class="back-link">Quellenverzeichnis &rarr;</a>
			</div>
			<p class="book-label">Evidenz-Audit &amp; Argumentationskette</p>
			<h1>Vom stillen Tod zur lauten Reform</h1>
			<p class="subtitle">
				Evidenz-Audit und Argumentationskette für
				<em>Boys Don't Cry</em>
			</p>
			<p class="subtitle-note">
				Ein journalistisch-populäres Buch. Die Stärke liegt in robusten britischen
				Gesundheitsstatistiken, die Schwäche im bewusst nicht-akademischen Zugriff auf die
				theoretische Rahmung — das ist Programm, nicht Zufall.
			</p>
		</div>
	</header>

	<section class="chain-section">
		<h2>Argumentationskette: Von der Sterblichkeitsstatistik zur Feminismus-Allianz</h2>
		<p class="chain-intro">
			Urwins Schritte &mdash; vom empirischen Ankerpunkt (britische Sterblichkeitsstatistiken)
			über die anti-essentialistische Rahmung und die frühe Konditionierung („Jungen weinen nicht")
			zum organisierenden Konzept der <em>toxischen Männlichkeit</em> mit ihrer aktiv/passiv-Unterscheidung,
			zu den institutionellen Reproduktionskanälen (Militär, Popkultur, Konsum-Werbung, Homophobie)
			und schließlich zu einer pragmatisch gestaffelten Lösungs-Strategie.
		</p>
		...
		<BdcArgumentGraph onNodeClick={(id) => (selectedNodeId = selectedNodeId === id ? null : id)} />
			...
			<div class="node-detail" style="--nd-color: {color}">
				...
				<div class="nd-evidence">
					...
					{#each selectedNode.evidenceRefs as ref, i}
						<a class="ev-link" href="/boys-dont-cry/references?search={encodeURIComponent(ref.authorSearch)}">{ref.label}</a>{#if i < selectedNode.evidenceRefs.length - 1}<span class="ev-sep">&middot;</span>{/if}
					{/each}
				</div>
	...
	<section class="audit-section">
		<h2>Evidenz-Audit: Was trägt der britische Männer-Journalismus?</h2>
		<p class="audit-intro">
			Urwin arbeitet mit einer Kombination aus robusten britischen Gesundheitsstatistiken
			(Samaritans, NHS, ONS), qualitativen Interviews (Huddleston, Bentley, Oliver, Reynolds,
			Sharples), persönlicher Autobiographie (Vatertod, Großvater-PTSD, eigene Depression) und
			Popkultur-Kritik (Top Gear, Fight Club, Lynx-Werbung, Hooliganismus).
		</p>
	...
	<footer class="app-footer">
		<p>
			Evidenz-Audit und Argumentationsanalyse zu <em>Boys Don't Cry — Identität, Gefühl und Männlichkeit</em>
			(Jack Urwin, Edition Nautilus 2017 / Iconbooks 2016). Das journalistisch-britische Pendant
			zu Süfkes klinischer Phänomenologie und Böhnischs akademischer Soziologie — gleicher Grundbefund,
			andere Gattung und Zielgruppe.
		</p>
	</footer>
</div>
...
<style>
	...
	.show-all-btn {
		...
		border-radius: 8px; color: #818cf8; font-size: 0.8rem;
		cursor: pointer; font-family: inherit;
	}
	.show-all-btn:hover { background: rgba(129, 140, 248, 0.2); }
