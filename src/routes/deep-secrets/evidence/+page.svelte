<script lang="ts">
	import { evidenceItems, evidenceStatusInfo, auditSummary } from '$lib/data/ds-evidence-audit-data';
	import { chain, getWeakestPath, chainSummary, typeLabels as chainTypeLabels } from '$lib/data/ds-argument-chain-data';
	import DsArgumentGraph from '$lib/components/DsArgumentGraph.svelte';
...
<svelte:head>
	<title>Evidenz-Audit &amp; Argumentationskette — Deep Secrets</title>
...
<div class="app">
	<header class="hero">
		<div class="hero-inner">
			<div class="nav-links">
				<a href="/deep-secrets" class="back-link">&larr; Buchübersicht</a>
				<a href="/deep-secrets/references" class="back-link">Quellenverzeichnis &rarr;</a>
			</div>
			<p class="book-label">Evidenz-Audit &amp; Argumentationskette</p>
			<h1>Von der 13-jährigen Intimität zum 18-jährigen Verlust</h1>
			<p class="subtitle">
				Evidenz-Audit und Argumentationskette für
				<em>Deep Secrets</em>
			</p>
			<p class="subtitle-note">
				Die stärkste longitudinale ethnographische Primärquelle der Bibliothek. 20+ Jahre Interviewstudien
				mit hunderten NYC-public-school-Jungen (Listening Guide, Brown/Gilligan-Tradition). Dieselben Jungen
				über Jahre hinweg verfolgt — die &bdquo;Mask&ldquo;/&bdquo;Boy Code&ldquo;-Konvergenz wird hier von klinischer Inferenz
				zu direkter longitudinaler Beobachtung aufgewertet.
			</p>
		</div>
	</header>

	<section class="chain-section">
		<h2>Argumentationskette: Von der longitudinalen Methodologie zur &bdquo;Schützen statt Lehren&ldquo;-Solution</h2>
		<p class="chain-intro">
			Way's Schritte &mdash; von der Methodologie (Listening Guide, 20-Jahre-Längsschnitt, ~135 Jungen)
			über die zentralen observational findings (boys 13&ndash;15 emotional hochartikuliert,
			dieselben boys 16&ndash;18 verlieren ihre Freundschaften) zu den mechanistischen Interpretationen
			(Homophobie als Kernmechanismus, Suizid-Timing, Brown/Gilligan-Parallele) und zur
			&bdquo;Schützen statt Lehren&ldquo;-Solution. Direct observational upgrade der klinischen
			Mask/Boy-Code-Konvergenz.
		</p>
		...
		<DsArgumentGraph onNodeClick={(id) => (selectedNodeId = selectedNodeId === id ? null : id)} />
			...
			<div class="node-detail" style="--nd-color: {color}">
				...
				<div class="nd-evidence">
					...
					{#each selectedNode.evidenceRefs as ref, i}
						<a class="ev-link" href="/deep-secrets/references?search={encodeURIComponent(ref.authorSearch)}">{ref.label}</a>{#if i < selectedNode.evidenceRefs.length - 1}<span class="ev-sep">&middot;</span>{/if}
					{/each}
				</div>
	...
	<section class="audit-section">
		<h2>Evidenz-Audit: Was trägt die longitudinale Listening-Guide-Methode?</h2>
		<p class="audit-intro">
			Way arbeitet mit 20+ Jahren longitudinaler Interviewstudien, der Listening-Guide-Methodologie
			(Brown/Gilligan), thick descriptions einzelner Jungen über Zeit, und parallelisierender
			unabhängiger Replikation (Chu bei weißen Privatschul-Jungen, Santos quantitative Validation).
			Die Kernbefunde sind direkte observational evidence &mdash; interpretative Brücken zur Homophobie,
			zum Suizid-Timing und zum Brown/Gilligan-Parallel bleiben unterschiedlich gut gestützt.
		</p>
	...
	<footer class="app-footer">
		<p>
			Evidenz-Audit und Argumentationsanalyse zu <em>Deep Secrets &mdash; Boys' Friendships and the Crisis of Connection</em>
			(Niobe Way, Harvard University Press 2011). Die stärkste longitudinale ethnographische
			Primärquelle des gesamten Bibliothek-Korpus. Way's Listening-Guide-Langzeitstudie wertet
			die &bdquo;Mask&ldquo;/&bdquo;Boy Code&ldquo;-Konvergenz (Pollack, Süfke, Kindlon/Thompson) von klinischer Inferenz
			zu direkter longitudinaler Beobachtung auf.
		</p>
	</footer>
</div>
...
<style>
	...
	.subtitle em { color: #fda4af; }
	.subtitle-note { color: #64748b; font-size: 0.85rem; margin: 0; max-width: 720px; margin-left: auto; margin-right: auto; line-height: 1.5; }
	...
	.show-all-btn {
		...
		border-radius: 8px; color: #f43f5e; font-size: 0.8rem;
		cursor: pointer; font-family: inherit;
	}
	.show-all-btn:hover { background: rgba(244, 63, 94, 0.2); }
