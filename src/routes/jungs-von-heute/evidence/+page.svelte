<script lang="ts">
	import { evidenceItems, evidenceStatusInfo, auditSummary } from '$lib/data/evidence-audit-data';
	import { chain, getWeakestPath, chainSummary, typeLabels as chainTypeLabels } from '$lib/data/argument-chain-data';
	import ArgumentGraph from '$lib/components/ArgumentGraph.svelte';
...
<svelte:head>
	<title>Evidenz-Audit &amp; Argumentationskette — Jungs von heute, Männer von morgen</title>
...
<div class="app">
	<header class="hero">
		<div class="hero-inner">
			<div class="nav-links">
				...
				<a href="/jungs-von-heute/references" class="back-link">Quellenverzeichnis</a>
				<a href="/jungs-von-heute/reading" class="back-link">Weiterführende Literatur &rarr;</a>
			</div>
			...
			<p class="subtitle">
				...
				<em>Jungs von heute, Männer von morgen</em>
			</p>
	...
	<section class="chain-section">
		...
		<p class="chain-intro">
			Die 13 Schritte der Buchlogik — von den Prämissen bis zu den Handlungsempfehlungen.
		...
		<!-- Interactive graph -->
		<ArgumentGraph onNodeClick={(id) => (selectedNodeId = selectedNodeId === id ? null : id)} />
			...
			<div class="node-detail" style="--nd-color: {color}">
				...
				<div class="nd-evidence">
					...
					{#each selectedNode.evidenceRefs as ref, i}
						<a class="ev-link" href="/jungs-von-heute/references?search={encodeURIComponent(ref.authorSearch)}">{ref.label}</a>{#if i < selectedNode.evidenceRefs.length - 1}<span class="ev-sep">&middot;</span>{/if}
					{/each}
				</div>
	...
	<footer class="app-footer">
		<p>
			Evidenz-Audit und Argumentationsanalyse zu <em>Jungs von heute, Männer von morgen</em> (Greiner, 2024).
...
<style>
	...
	.show-all-btn {
		...
		border-radius: 8px; color: #a5b4fc; font-size: 0.8rem;
		cursor: pointer; font-family: inherit;
	}
	.show-all-btn:hover { background: rgba(99, 102, 241, 0.2); }
