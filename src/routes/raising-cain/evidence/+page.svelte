<script lang="ts">
	import { evidenceItems, evidenceStatusInfo, auditSummary } from '$lib/data/rc-evidence-audit-data';
	import { chain, getWeakestPath, chainSummary, typeLabels as chainTypeLabels } from '$lib/data/rc-argument-chain-data';
	import RcArgumentGraph from '$lib/components/RcArgumentGraph.svelte';
...
<svelte:head>
	<title>Evidenz-Audit &amp; Argumentationskette — Raising Cain</title>
...
<div class="app">
	<header class="hero">
		<div class="hero-inner">
			<div class="nav-links">
				<a href="/raising-cain" class="back-link">&larr; Buchübersicht</a>
				<a href="/raising-cain/references" class="back-link">Quellenverzeichnis &rarr;</a>
			</div>
	...
	<section class="chain-section">
		...
		<!-- Interactive graph -->
		<RcArgumentGraph onNodeClick={(id) => (selectedNodeId = selectedNodeId === id ? null : id)} />
			...
			<div class="node-detail" style="--nd-color: {color}">
				...
				<div class="nd-evidence">
					...
					{#each selectedNode.evidenceRefs as ref, i}
						<a class="ev-link" href="/raising-cain/references?search={encodeURIComponent(ref.authorSearch)}">{ref.label}</a>{#if i < selectedNode.evidenceRefs.length - 1}<span class="ev-sep">&middot;</span>{/if}
					{/each}
				</div>
	...
	<footer class="app-footer">
		<p>
			Evidenz-Audit und Argumentationsanalyse zu <em>Raising Cain</em> (Kindlon &amp; Thompson, 1999).
...
<style>
	...
	.show-all-btn {
		...
		border-radius: 8px; color: #2dd4bf; font-size: 0.8rem;
		cursor: pointer; font-family: inherit;
	}
	.show-all-btn:hover { background: rgba(20, 184, 166, 0.2); }
